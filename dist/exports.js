...
export function extractNames(node) {
    ...
    if (node.type === 'ArrayPattern') {
        return node.elements.filter((el) => el !== null).flatMap(extractNames);
    }
 ...
 *  - export * / export * as
 */
export function extractNamedValueExports(ast) {
    const names = [];
 ...
 */
export function findCompoundObjectProperties(ast, namespaceName) {
    for (const node of ast.body) {
        if (node.type !== 'ExportNamedDeclaration')
            continue;
        const decl = node.declaration;
        ...
        if (d.id.type !== 'Identifier' || d.id.name !== namespaceName)
            continue;
...
export function findLocalDeclarations(ast, names) {
